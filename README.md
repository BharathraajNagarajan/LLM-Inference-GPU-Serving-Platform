...
This section is an inventory of what actually happens today, observed directly from `src/server/main.py` and the benchmark harness — not a roadmap of handling that has been built.

- **No retry or backoff, on either side.** The server does not retry anything internally, and the benchmark harness (`benchmark/run_benchmark.py`) does not retry a failed request either — it records the failure (`RequestRecord.success = False`) and moves on. A dropped or non-200 request is simply lost.
- **No graceful degradation if the model fails to load.** The `lifespan` hook in `src/server/main.py` calls `AutoModelForCausalLM.from_pretrained(...)` with no `try`/`except` around it. If that call fails for any reason (network failure fetching weights, CUDA out-of-memory, a corrupted cache), the app never finishes startup and never serves any request — including `/health`, since Uvicorn does not begin accepting connections at all until the lifespan startup completes (see the probe comments in `k8s/deployment.yaml`). There is no fallback path beyond the one CUDA-unavailable-at-runtime case that already exists (falling back to CPU with a printed warning — see [Section 5](#5-gpu-setup)); a load failure for any other reason is a hard, silent-to-the-caller stall, not a handled error.
- **No request timeout handling on the server.** `model.generate()` runs to completion with whatever `max_new_tokens`/`max_tokens` the caller asked for; there is no server-side wall-clock cutoff. A request asking for an unusually large token count simply runs for as long as it takes, holding `generate_lock` the entire time.
- **`generate_lock` means the server is intentionally, fully serialized under load.** This is documented in `src/server/main.py` (see the comment above `generate_lock`), not an accidental bottleneck: exactly one `model.generate()` call runs at a time, regardless of how many concurrent requests arrive. This is a real scalability ceiling for the naive baseline, and it is precisely the reason vLLM is planned as the next step (see [Section 1](#1-problem) and [Section 13](#13-future-work)) — not something intended to be "fixed" within this baseline itself.
- **No authentication or rate-limiting on any endpoint.** `/generate`, `/v1/chat/completions`, and `/health` all accept unauthenticated requests from anyone who can reach the server, with no per-caller or global request-rate limit.
- **No input validation beyond Pydantic's type-level checks.** Nothing caps prompt length or the requested `max_new_tokens`/`max_tokens`; a caller can request an arbitrarily large generation and the server will attempt it.
- **The benchmark harness's `MAX_VERIFIED_CONCURRENCY = 4` cap is a benchmarking safety check, not a production admission-control mechanism.** It stops the *benchmark harness* from sending untested concurrency levels at the server (see [Section 7](#7-benchmark-methodology)); the server itself enforces no concurrency limit and will accept and queue — behind `generate_lock` — however many concurrent HTTP requests actually arrive.

## 12. Limitations

- **No vLLM integration exists yet.** Every result and behavior described in this README — the benchmark numbers in [Section 8](#8-actual-benchmark-results), the GPU telemetry, the OpenAI-compatible endpoint — comes from the naive synchronous PyTorch/FastAPI baseline (`backend: "naive_fastapi_baseline"`). It is the only serving path that has actually been built and exercised in this project so far.
- **Benchmark results are a single run each, with a small sample size (6 requests per concurrency level).** There is no repetition and no variance/confidence interval; see the caveats already documented in [Section 8](#8-actual-benchmark-results) rather than repeated here. These numbers should not be read as statistically robust.
- **Kubernetes manifests and the Dockerfile are written, not deployed or built.** `k8s/deployment.yaml`/`k8s/service.yaml` have been syntax-validated only, against no live cluster; `docker/Dockerfile` has been written and manually reviewed only, with `docker build` never run against it. See the status notes in [Section 10](#10-kubernetes-deployment) and the Docker note in [Section 4](#4-local-setup) for the current, honest status of each rather than repeated here.
- **Only tested on one model, one GPU.** All measurements in this repo are from `Qwen/Qwen2.5-0.5B-Instruct` on a single NVIDIA GeForce GTX 1050 (3GB VRAM, see [Section 5](#5-gpu-setup)). Nothing here says anything about behavior with larger models, multiple GPUs, or different hardware.
- **Narrow, synthetic prompt distribution.** The benchmark harness uses a fixed pool of six short, factual, single-turn prompts (see [Section 7](#7-benchmark-methodology)) — this says nothing about longer prompts, multi-turn conversations, or more varied real-world traffic.
- **No streaming, and only a minimal OpenAI-compatible surface.** Both `/generate` and `/v1/chat/completions` return the full completion synchronously, not token-by-token. `/v1/chat/completions` supports only `model`, `messages`, and `max_tokens`; decoding is hard-coded server-side to greedy (`do_sample=False`) regardless of what a caller requests — there is no `temperature`, `top_p`, `n`, or other sampling-parameter support.
